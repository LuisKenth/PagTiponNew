...
export default function ProvincialSidebar() {
  ...
  const isActiveLink = (href: string) => {
    ...
    const createEventsPath = "/dashboard/provincial/events/create";
    const provincialEventsPath = "/dashboard/provincial/events";
    ...
    if (targetPath === provincialEventsPath) {
      return (
        ...
        (currentPath.startsWith(`${provincialEventsPath}/`) &&
          !currentPath.startsWith(createEventsPath))
      );
    }
  ...
  return (
    <>
      <div className="sticky top-0 z-40 flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 lg:hidden">
        <div>
          <h1 className="text-lg font-bold text-slate-900">PagTipon</h1>
          <p className="text-xs text-slate-500">Provincial Admin</p>
        </div>
      ...
      >
        <div className="flex h-full flex-col">
          <div className="border-b border-slate-200 p-6">
            <h1 className="text-2xl font-bold text-slate-900">PagTipon</h1>
            <p className="mt-1 text-sm text-slate-500">Provincial Admin</p>
          </div>
